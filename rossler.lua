...
function bogacki_shampine(eqns, ivs, tol, st, fi)
  local vals, kvals = {{}}, {{}, {}, {}, {}}
  ...
  repeat
    ...
    if err < tol then -- it worked!
      ...
      ts[cnt] = t+h
      vals[cnt] = {}
      for i = 1, vn do vals[cnt][i] = nest[i] end
    else
      print("failed: " .. h .. " " .. err)
    end
    h = 0.8 * h * (tol / err)^(1/4)
...
rossler = {
function(t, xt) return -xt[2] - xt[3] end,
function(t, xt) return xt[1] + 0.2 * xt[2] end,
function(t, xt) return 0.2 + xt[3] * (xt[1] - 5.7) end
}
function complexlorenz(rho1, rho2, b, sigma, e)
  return {
function(t, xt) return -sigma*xt[1] + sigma*xt[3] end,
function(t, xt) return -sigma*xt[2] + sigma*xt[4] end,
function(t, xt) return (rho1 - xt[5])*xt[1] - rho2*xt[2] - xt[3] - e*xt[4] end,
function(t, xt) return rho2*xt[1] + (rho1 - z)*xt[2] + e*xt[3] - xt[4] end,
function(t, xt) return -b*xt[5] + xt[1]*xt[3] + xt[2]*xt[4] end
}
end

test_times, test_vals = bogacki_shampine(rossler, {1, 1, 1}, 0.00001, 0, 6250)
